...
const I18N = {
  ar: {
    ...
    sharpModeTitle: 'وضع الحدّة على الشاشات المكبّرة',
    sharpModeDesc: 'يجعل الدائرة واللوحة حادّتين على شاشات 125% فأعلى. تجريبي — يُطبَّق بعد إعادة تشغيل البرنامج.',
    ...
    suggestExeMessage: 'تم العثور على ملف تشغيل داخل "{folder}": {file}. هل تريد إضافته كلعبة؟',
    suggestExeAdd: 'إضافة',
    exeListCounter: 'المحدد: {n} من {total}',
    exeListMainBadge: 'رئيسي محتمل',
    importGamesBtn: 'استيراد الألعاب',
    scanFolderBtn: 'فحص مجلد',
    scanFolderSub: 'يشمل المجلدات الفرعية',
    tipFavoriteAdd: 'إضافة للمفضلة',
    tipFavoriteRemove: 'إزالة من المفضلة',
    tipShownInRadial: 'ظاهرة في دائرة الألعاب',
    tipHiddenFromRadial: 'مخفية من دائرة الألعاب',
    tipCompanionsCount: 'عدد البرامج المرافقة لهذه اللعبة',
    tipOpenFolder: 'فتح مجلد اللعبة',
    tipGameDetails: 'تفاصيل اللعبة وإعداداتها',
    tipRemoveGame: 'حذف اللعبة من القائمة',
    tipRemoveCompanion: 'حذف البرنامج المرافق',
    tipQuickSlot: 'رقم الاختصار السريع: اضغطه داخل الدائرة لفتح اللعبة',
    tipBoost: 'Boost مفعّل لهذه اللعبة',
    tipUwp: 'تطبيق من قائمة ابدأ أو المتجر (UWP)',
    tipMuted: 'صوت اللعبة مكتوم',
    tipMissing: 'ملف اللعبة غير موجود في هذا المسار',
    tipQuickAccessGame: 'الانتقال إلى خيارات اللعبة',
    tipQuickAccessComp: 'الانتقال إلى إعدادات هذا البرنامج المرافق',
    importModalTitle: 'ألعابك المثبّتة',
    importModalSubtitle: 'تم العثور على {count} لعبة ({fresh} جديدة) — المصادر: Steam · Epic · GOG · Ubisoft · Games',
    importAddedBadge: 'مضافة',
    dropOverlayTitle: 'أفلت هنا لإضافة اللعبة',
    dropOverlayHint: 'ملف exe أو اختصار أو مجلد ألعاب كامل',
    emptyGamesTitle: 'ابدأ بإضافة ألعابك',
    emptyGamesDesc: 'اسحب ملف اللعبة أو مجلد ألعابك كاملاً وأفلته هنا، أو استخدم الأزرار بالأعلى.',
  ...
  en: {
    ...
    sharpModeTitle: 'Sharp mode on scaled displays',
    sharpModeDesc: 'Makes the radial and panel sharp on 125%+ displays. Experimental — restart the app to apply.',
    ...
    suggestExeMessage: 'Found an executable inside "{folder}": {file}. Add it as a game?',
    suggestExeAdd: 'Add',
    exeListCounter: 'Selected: {n} of {total}',
    exeListMainBadge: 'Likely main',
    importGamesBtn: 'Import games',
    scanFolderBtn: 'Scan folder',
    scanFolderSub: 'Includes subfolders',
    tipFavoriteAdd: 'Add to favorites',
    tipFavoriteRemove: 'Remove from favorites',
    tipShownInRadial: 'Shown in the radial menu',
    tipHiddenFromRadial: 'Hidden from the radial menu',
    tipCompanionsCount: 'Companion programs for this game',
    tipOpenFolder: 'Open game folder',
    tipGameDetails: 'Game details & settings',
    tipRemoveGame: 'Remove from the list',
    tipRemoveCompanion: 'Remove companion program',
    tipQuickSlot: 'Quick launch number: press it inside the radial to launch',
    tipBoost: 'Boost is on for this game',
    tipUwp: 'Start menu / Store app (UWP)',
    tipMuted: 'Game audio is muted',
    tipMissing: 'Game file not found at this path',
    tipQuickAccessGame: 'Jump to the game options',
    tipQuickAccessComp: "Jump to this companion's settings",
    importModalTitle: 'Your installed games',
    importModalSubtitle: 'Found {count} games ({fresh} new) — sources: Steam · Epic · GOG · Ubisoft · Games',
    importAddedBadge: 'Added',
    dropOverlayTitle: 'Drop here to add the game',
    dropOverlayHint: 'An exe, a shortcut, or a whole games folder',
    emptyGamesTitle: 'Start by adding your games',
    emptyGamesDesc: 'Drag a game file or a whole games folder and drop it here, or use the buttons above.',
...
let state = { lang: 'ar', hotkey: '—', muteHotkey: '—', muteEnabled: false,
              ...
              hubAlwaysVisible: false, isAdmin: false, radialNoGlow: false, sharpMode: false,
...
window.chrome.webview.addEventListener('message', (e) => {
  ...
  if (msg.type === 'state') { state = msg; onStateUpdated(); document.documentElement.style.visibility = ''; }
  ...
  else if (msg.type === 'suggestExeList') { showExeListModal(msg.folder, msg.items || [], !!msg.library); }
  else if (msg.type === 'installedGames') { showImportModal(msg.items || []); }
...
function showExeListModal(folderName, items, library) {
  exeListItems = items;
  exeListSelected = new Set();
  // مجلد مكتبة (ألعاب كثيرة): كل لعبة مؤشّرة مسبقاً؛ وإلا فقط الملفات الأشبه بالرئيسية
  if (library) items.forEach((it, i) => exeListSelected.add(i));
  else items.forEach((it, i) => { if (it.main) exeListSelected.add(i); });
  if (exeListSelected.size === 0 && items.length > 0) exeListSelected.add(0);
  const titleEl = document.getElementById('exeListTitleEl');
  titleEl.setAttribute('data-t', 'exeListTitle');
  titleEl.innerText = t('exeListTitle');
    ...
    row.innerHTML = `
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2">
          <p class="text-xs font-bold text-white truncate">${escapeHtml(it.label || it.name)}</p>
          ${it.main && !library ? `<span class="main-badge">${escapeHtml(t('exeListMainBadge'))}</span>` : ''}
        </div>
  ...
  const onOk = () => {
    const paths = [];
    const names = [];
    exeListSelected.forEach(i => { if (exeListItems[i]) { paths.push(exeListItems[i].path); names.push(exeListItems[i].label || ''); } });
    close(); cleanup();
    if (paths.length > 0) nativeAction('addSelectedExes', { paths, names });
  };
  function cleanup() { cancelBtn.removeEventListener('click', onCancel); okBtn.removeEventListener('click', onOk); }
  cancelBtn.addEventListener('click', onCancel);
  okBtn.addEventListener('click', onOk);
}
const IMPORT_SOURCE_COLORS = { Steam: '#66c0f4', Epic: '#e5e7eb', GOG: '#a78bfa', Ubisoft: '#38bdf8', Games: '#34d399' };
function showImportModal(items) {
  exeListItems = items;
  exeListSelected = new Set();
  items.forEach((it, i) => { if (!it.added) exeListSelected.add(i); });
  const titleEl = document.getElementById('exeListTitleEl');
  titleEl.removeAttribute('data-t');      // حتى لا تعيد الترجمة الثابتة عنوان القائمة القديم
  titleEl.innerText = t('importModalTitle');
  const fresh = items.filter(it => !it.added).length;
  document.getElementById('exeListSubtitle').innerText = tFormat('importModalSubtitle', { count: items.length, fresh });
  const container = document.getElementById('exeListContainer');
  container.innerHTML = '';
  items.forEach((it, i) => {
    const row = document.createElement('div');
    ...
    row.dataset.idx = i;
    row.tabIndex = 0;
    if (it.added) row.style.opacity = '0.5';
    const color = IMPORT_SOURCE_COLORS[it.source] || '#94a3b8';
    const iconHtml = it.icon
      ? `<img src="${it.icon}" class="w-8 h-8 rounded object-contain bg-black/30 border border-white/10 shrink-0" onerror="this.style.display='none'">`
      : `<div class="w-8 h-8 rounded bg-black/30 border border-white/10 shrink-0 flex items-center justify-center text-gray-400"><i class="fa-solid fa-gamepad"></i></div>`;
    row.innerHTML = `
      <div class="chk"><i class="fa-solid fa-check"></i></div>
      ${iconHtml}
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2">
          ...
          <span class="main-badge" style="background: rgba(255,255,255,0.08); color: ${color}; border-color: ${color}55;">${escapeHtml(it.source)}</span>
          ${it.added ? `<span class="main-badge">${escapeHtml(t('importAddedBadge'))}</span>` : ''}
        </div>
        <p class="text-[10px] text-gray-500 font-mono truncate" dir="ltr">${escapeHtml(it.path)}</p>
      </div>`;
    row.onclick = () => {
  ...
  const onOk = () => {
    const indices = [];
    exeListSelected.forEach(i => { if (exeListItems[i] && !exeListItems[i].added) indices.push(exeListItems[i].i); });
    close(); cleanup();
    if (indices.length > 0) nativeAction('addImportedGames', { indices });
  };
  function cleanup() { cancelBtn.removeEventListener('click', onCancel); okBtn.removeEventListener('click', onOk); }
  cancelBtn.addEventListener('click', onCancel);
  okBtn.addEventListener('click', onOk);
...
function exeListSelectAll(select) {
  exeListSelected = new Set();
  if (select) exeListItems.forEach((it, i) => { if (!it.added) exeListSelected.add(i); });
  const container = document.getElementById('exeListContainer');
...
function onStateUpdated() {
  ...
  const sharpToggle = document.getElementById('sharpModeToggle');
  if (sharpToggle) sharpToggle.classList.toggle('on', !!state.sharpMode);
...
function renderGamesList() {
  ...
  if (emptyState) emptyState.classList.toggle('hidden', state.games.length !== 0);
  ...
  filtered.forEach(game => {
    ...
    const eyeIcon = game.showInRadial
      ? `<i class="fa-solid fa-eye text-emerald-400 text-xs" title="${escapeHtml(t('tipShownInRadial'))}"></i>`
      : `<i class="fa-solid fa-eye-slash text-gray-500 text-xs" title="${escapeHtml(t('tipHiddenFromRadial'))}"></i>`;
    const uwpBadge = game.isUwp ? `<span class="uwp-badge" title="${escapeHtml(t('tipUwp'))}">UWP</span>` : '';
    ...
    const muteIndicator = game.muted ? `<span class="mute-badge" title="${escapeHtml(t('tipMuted'))}"><i class="fa-solid fa-volume-xmark"></i></span>` : '';
    ...
    const slotBadge = (game.quickSlot && game.quickSlot >= 1 && game.quickSlot <= 9)
      ? `<span class="slot-badge" title="${escapeHtml(t('tipQuickSlot'))}">${game.quickSlot}</span>` : '';
    const boostBadge = game.boostFps ? `<span class="boost-badge" title="${escapeHtml(t('tipBoost'))}"><i class="fa-solid fa-rocket"></i></span>` : '';
    ...
    card.innerHTML = `
      <div class="flex items-center gap-3 overflow-hidden">
        ...
        <div class="relative shrink-0">
          ...
          ${game.missing ? `<span class="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-amber-500 border-2 border-[#0f0c1b] flex items-center justify-center" title="${escapeHtml(t('tipMissing'))}"><i class="fa-solid fa-triangle-exclamation text-[9px] text-black"></i></span>` : ''}
        </div>
      ...
      <div class="flex items-center gap-1 shrink-0">
        <button onclick="event.stopPropagation(); toggleFavoriteByIndex(${game.index})" class="${favClass}" tabindex="-1" title="${escapeHtml(t(game.favorite ? 'tipFavoriteRemove' : 'tipFavoriteAdd'))}"><i class="fa-solid fa-star text-sm"></i></button>
        ${eyeIcon}
        <span class="text-[11px] px-2 py-0.5 rounded-full ${game.companions.length > 0 ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30' : 'bg-gray-800 text-gray-400'}" title="${escapeHtml(t('tipCompanionsCount'))}">${game.companions.length}</span>
        ${canOpenFolder ? `<button onclick="event.stopPropagation(); nativeAction('openGameFolder', {index: ${game.index}})" class="icon-action-btn" tabindex="-1" title="${escapeHtml(t('tipOpenFolder'))}"><i class="fa-solid fa-folder-open text-xs"></i></button>` : ''}
        <button onclick="event.stopPropagation(); openGameDetails(${game.index})" class="text-gray-400 hover:text-cyan-300 p-1" tabindex="-1" title="${escapeHtml(t('tipGameDetails'))}"><i class="fa-solid fa-gears"></i></button>
        <button data-name="${escapeHtml(game.name)}" onclick="event.stopPropagation(); confirmRemoveGame(${game.index}, this.dataset.name)" class="text-gray-400 hover:text-red-400 p-1" tabindex="-1" title="${escapeHtml(t('tipRemoveGame'))}"><i class="fa-solid fa-xmark"></i></button>
      </div>`;
...
function renderCompanions() {
  ...
  game.companions.forEach(comp => {
    ...
    item.innerHTML = `
      ...
      <button data-name="${escapeHtml(comp.name)}" onclick="confirmRemoveCompanion(${game.index}, ${comp.index}, this.dataset.name)" class="text-gray-400 hover:text-red-400 p-1 text-xs shrink-0" tabindex="0" title="${escapeHtml(t('tipRemoveCompanion'))}"><i class="fa-solid fa-trash"></i></button>`;
...
function renderQuickAccessBar(game) {
  // شريط الوصول السريع يفيد فقط للتنقل بين اللعبة وبرامجها المرافقة: نخفيه لو ما فيه مرافق
  const qaBar = document.getElementById('quickAccessBar');
  if (qaBar) qaBar.classList.toggle('hidden', !(game.companions && game.companions.length > 0));
  ...
  gameChip.title = t('tipQuickAccessGame');
  ...
  (game.companions || []).forEach(c => {
    ...
    chip.title = t('tipQuickAccessComp');
...
function renderBlackBoxSection(game) {
  ...
  if (enabled) {
    ...
    if (!state.isAdmin) {
      ...
      // بصلاحيات مسؤول لا يوجد ما يستدعي تنبيهاً (ميزة حرارة المعالج غير مدعومة، فلا تنبيه لها)
...
function renderPerfCharts(report) {
  ...
  if (typeof ApexCharts === 'undefined') {
    if (!window.__apexLoading) {
      ...
      script.src = '__GL_ASSETS__ui_apexcharts.js';   // نسخة محلية إن وُجدت (مجلد ui_assets)
      script.onload = () => { window.__apexLoading = false; renderPerfCharts(report); };
      script.onerror = () => {
        // لا توجد نسخة محلية: نجرّب الـ CDN (مثبّتة على الإصدار 6 بدون تحديث مفاجئ لإصدار جديد)
        const cdn = document.createElement('script');
        cdn.src = 'https://cdn.jsdelivr.net/npm/apexcharts@6';
        cdn.onload = () => { window.__apexLoading = false; renderPerfCharts(report); };
        cdn.onerror = () => { window.__apexLoading = false; console.warn('[Charts] Failed to load ApexCharts'); };
...
function toggleSharpMode() { nativeAction('setSharpMode', { enabled: state.sharpMode ? 0 : 1 }); }
