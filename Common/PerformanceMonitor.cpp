...
// ============================================================
// لا نربط nvml.lib — نحمّله ديناميكيًا من تعريف NVIDIA.
// إذا لم يوجد (AMD/Intel)، تعمل المراقبة بدون GPU.
// ============================================================

// أنواع NVML الأساسية (مطابقة لتعريف NVIDIA الرسمي)
...
void PerformanceMonitor::InitNvml() {
    ...
    // جرّب مسارات NVML المعروفة
    m_nvml->hDll = LoadLibraryW(L"nvml.dll");
    if (!m_nvml->hDll) {
...
bool PerformanceMonitor::StartSession(const std::wstring& gamePath,
    ...
    // ✅ ابدأ ETW FPS خارج الـ lock (عملية قد تستغرق قليلاً)
    if (gamePid != 0) {
...
void PerformanceMonitor::StopSession() {
    // ✅ أوقف ETW أولًا (خارج الـ lock، لأن Stop() قد ينتظر thread)
    if (m_etwFps) {
        m_etwFps->Stop();
    }
...
bool PerformanceMonitor::WriteReportLocked(const PerformanceReport& r) const {
    ...
    std::wofstream f(tmpPath.c_str(), std::ios::trunc);
...
std::vector<SessionMeta> PerformanceMonitor::ListSessions(const std::wstring& gamePath) const {
    ...
    for (auto& name : files) {
        ...
        if (headerFound) result.push_back(meta);
    }
    return result;
}
...
bool PerformanceMonitor::DeleteAllSessions(const std::wstring& gamePath) const {
    ...
    FindClose(h);
    bool ok = true;
    for (auto& p : toDel) if (!DeleteFileW(p.c_str())) ok = false;
    return ok;
}
