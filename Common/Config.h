...
#include <string>
#include <vector>
#include "Utf8Stream.h"   // قراءة/كتابة UTF-8 (بديل wifstream/wofstream)
...
// ✅ أنماط الرسم المعرّفة في الكود (الظاهر منها في الواجهة 6 فقط — انظر kVisibleRadialStyles في Config.cpp)
...
struct ExeScanResult {
    ...
    std::wstring displayName;   // اسم لطيف للعرض (مثل اسم مجلد اللعبة)؛ فارغ = يُشتق من اسم الملف
};
...
// وضع الحدّة على الشاشات المكبّرة (125%+) — تجريبي ومطفي افتراضياً، يُطبَّق بعد إعادة تشغيل البرنامج.
// GetDpiScale() يرجع 1.0 دائماً إذا الوضع غير مفعّل (فيبقى السلوك الأصلي حرفياً).
bool LoadSharpMode();
void SaveSharpMode(bool enabled);
void SetSharpModeActive(bool active);   // يُستدعى من wWinMain بعد SetProcessDPIAware
float GetDpiScale();
