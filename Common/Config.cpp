...
static const wchar_t* WELCOME_FILE = L"welcome_shown.txt";
...
// يمنع أي حرف فاصل أو سطر جديد داخل الحقول من إتلاف بنية games.cfg
// (الحقول العادية لا تتغير إطلاقاً — يُستبدل فقط الحرف المخرِّب بمسافة)
static std::wstring CfgField(const std::wstring& in) {
    auto bad = [](wchar_t c) {
        return c == FIELD_SEP || c == ITEM_SEP || c == SUB_SEP || c == L'\n' || c == L'\r';
    };
    bool hasBad = false;
    for (wchar_t c : in) if (bad(c)) { hasBad = true; break; }
    if (!hasBad) return in;
    std::wstring out = in;
    for (auto& c : out) if (bad(c)) c = L' ';
    return out;
}

static std::wstring SerializeCompanion(const CompanionEntry& c) {
    if (!c.runAsAdmin && c.launchArgs.empty() && c.showInRadial) return CfgField(c.path);
    return CfgField(c.path) + SUB_SEP + (c.runAsAdmin ? L"1" : L"0") + SUB_SEP +
        CfgField(c.launchArgs) + SUB_SEP + (c.showInRadial ? L"1" : L"0");
}
...
void LoadGames(std::vector<GameEntry>& games) {
    ...
    while (std::getline(f, line)) {
        ...
        if (fields.size() >= 18 && !fields[17].empty()) {
            ...
            if (prio >= 0 && prio <= 5) g.processPriority = ProcessPriorityFromIndex(prio);
        }
...
void SaveGames(const std::vector<GameEntry>& games) {
    ...
    bool wroteOk = false;
    {
        ...
        for (auto& g : games) {
            ...
            f << CfgField(g.name) << FIELD_SEP << CfgField(g.exePath) << FIELD_SEP
                << (unsigned long)g.color << FIELD_SEP << comp << FIELD_SEP
                << CfgField(g.iconPath) << FIELD_SEP << g.iconIndex << FIELD_SEP
                << (g.autoLangSwitch ? 1 : 0) << FIELD_SEP
                << (g.runAsAdmin ? 1 : 0) << FIELD_SEP
                << CfgField(g.launchArgs) << FIELD_SEP
                ...
                << CfgField(g.radialBgPath) << FIELD_SEP
    ...
    // لو فشلت الكتابة (قرص ممتلئ...) لا نستبدل الملف السليم بملف ناقص
    if (!wroteOk) { DeleteFileW(tmp.c_str()); return; }
    // نسخة من آخر إصدار سليم قبل الاستبدال (games.cfg.bak) — للاسترجاع اليدوي عند أي مشكلة
    CopyFileW(target.c_str(), (target + L".bak").c_str(), FALSE);
...
MuteSettings LoadMuteSettings() {
    MuteSettings ms;
    Utf8In f(MuteSettingsPath().c_str());
...
ControllerSettings LoadControllerSettings() {
    ...
    Utf8In f(ControllerSettingsPath().c_str());
...
OverlaySettings LoadOverlaySettings() {
    OverlaySettings os;
    Utf8In f(OverlaySettingsPath().c_str());
...
bool LoadPerformanceGlobalEnabled() {
    Utf8In f(PerformanceSettingsPath().c_str());
...
// ملاحظة أمان: أولوية Realtime تجمّد النظام مع الألعاب (تُجوّع خيوط الماوس/الكيبورد)،
// لذلك لم تعد تُعرض في الواجهة (5 خيارات: Normal..High). أي قيمة Realtime قديمة تُعامل High.
int ProcessPriorityCount() { return 5; }
ProcessPriority ProcessPriorityFromIndex(int idx) {
    if (idx < 0 || idx > 5) return ProcessPriority::Normal;
    if (idx == (int)ProcessPriority::Realtime) return ProcessPriority::High;
...
Lang LoadLanguage() {
    Utf8In f(LanguagePath().c_str());
    if (!f.is_open()) {
        // لا يوجد ملف لغة محفوظ:
        //  - مستخدم قديم (عنده إعدادات محفوظة) => العربية كما كانت، حتى لا تتغيّر لغته بعد التحديث.
        //  - مستخدم جديد => الإنجليزية (أغلب المستخدمين أجانب). نحفظ القرار فوراً حتى لا يتبدّل لاحقاً.
        const bool existingUser =
            GetFileAttributesW(FirstRunDonePath().c_str()) != INVALID_FILE_ATTRIBUTES ||
            GetFileAttributesW(ConfigPath().c_str()) != INVALID_FILE_ATTRIBUTES;
        const Lang def = existingUser ? Lang::AR : Lang::EN;
        SaveLanguage(def);
        return def;
    }
    std::wstring line; std::getline(f, line);
...
bool LoadAutoLayoutSwitch() {
    Utf8In f(AutoLayoutSwitchPath().c_str());
    if (!f.is_open()) return false;
    std::wstring line; std::getline(f, line);
    return TrimString(line) == L"1";
}
...
bool ActivateUwpApp(const std::wstring& aumid) {
    ...
    // بديل: برامج سطح المكتب التي لها AUMID (Chrome / Edge / Epic Launcher...) ترفضها ActivateApplication
    // (تعمل فقط مع تطبيقات الحزم)، لكن shell:AppsFolder يشغّل النوعين. يصلح أيضاً الإدخالات القديمة المحفوظة بـ uwp:
...
static std::wstring TryExtractAumidFromShortcut(const ShortcutInfo& si) {
    if (!si.aumid.empty()) {
        // AUMID تطبيقات UWP دائماً بصيغة PackageFamilyName!AppId (فيها "!").
        // برامج سطح المكتب (Chrome / Edge / Epic Launcher...) قد يحمل اختصارها AUMID بسيطاً مثل "Chrome":
        // هذي ليست UWP ولا تعمل مع ActivateApplication، فنشغّلها من الملف التنفيذي في الاختصار.
        const bool packaged = si.aumid.find(L'!') != std::wstring::npos;
        const bool hasRealTarget = !si.target.empty() &&
            ToLowerCopy(si.target).find(L"explorer.exe") == std::wstring::npos;
        if (packaged || !hasRealTarget) return si.aumid;
    }
...
float LoadRadialTransparency() {
    Utf8In f(RadialTransparencyPath().c_str());
...
std::wstring LoadPanelPreset() {
    Utf8In f(PanelPresetPath().c_str());
...
CustomTheme LoadCustomTheme() {
    CustomTheme ct;
    Utf8In f(CustomThemePath().c_str());
...
CustomRadialStyle LoadCustomRadialStyle() {
    ...
    Utf8In f(CustomRadialStylePath().c_str());
    if (!f.is_open()) return cs;
    std::wstring line;
    while (std::getline(f, line)) {
        line = TrimString(line);
...
RadialScale LoadRadialScale() {
    RadialScale rs;
    Utf8In f(RadialScalePath().c_str());
...
// شاشة الترحيب (اللوحة + الدائرة) تظهر مرة واحدة فقط لمستخدم جديد
bool IsWelcomeShown() {
    return GetFileAttributesW((GetAppDataDir() + L"\\" + WELCOME_FILE).c_str()) != INVALID_FILE_ATTRIBUTES;
}
void MarkWelcomeShown() {
    std::wstring target = GetAppDataDir() + L"\\" + WELCOME_FILE;
    std::wstring tmp = target + L".tmp";
    {
        Utf8Out f(tmp.c_str(), std::ios::trunc);
        if (f.is_open()) f << L"1\n";
...
bool LoadHubAlwaysVisible() {
    Utf8In f(HubAlwaysVisiblePath().c_str());
    if (!f.is_open()) return false;
    std::wstring line; std::getline(f, line);
    return TrimString(line) == L"1";
}
...
bool LoadPanelGlassEffect() {
    Utf8In f(PanelGlassEffectPath().c_str());
    if (!f.is_open()) return false;
    std::wstring line; std::getline(f, line);
    return TrimString(line) == L"1";
}
...
bool LoadRadialNoGlow() {
    Utf8In f(RadialNoGlowPath().c_str());
    if (!f.is_open()) return false;
    std::wstring line; std::getline(f, line);
    return TrimString(line) == L"1";
}
...
// ---------- وضع الحدّة على الشاشات المكبّرة (اختياري) ----------
static std::wstring SharpModePath() { return GetAppDataDir() + L"\\sharp_mode.cfg"; }

bool LoadSharpMode() {
    Utf8In f(SharpModePath().c_str());
    if (!f.is_open()) return false;
    std::wstring line; std::getline(f, line);
    return TrimString(line) == L"1";
}
void SaveSharpMode(bool enabled) {
    std::wstring target = SharpModePath();
    std::wstring tmp = target + L".tmp";
    {
        Utf8Out f(tmp.c_str(), std::ios::trunc);
        if (!f.is_open()) return;
...
static bool g_sharpModeActive = false;
void SetSharpModeActive(bool active) { g_sharpModeActive = active; }

float GetDpiScale() {
    if (!g_sharpModeActive) return 1.0f;      // الوضع مطفي: كل الحسابات كما كانت تماماً
    static const float s = []() -> float {
...
void AppendPlaySession(const std::wstring& gamePath,
    ...
    if (GetFileAttributesW(metaPath.c_str()) == INVALID_FILE_ATTRIBUTES) {
        ...
        {
            Utf8Out mf(metaTmp.c_str(), std::ios::trunc);
            if (mf.is_open()) {
                mf << gamePath << L"\n" << gameName << L"\n";
                mf.flush();
            }
...
std::vector<PlaySessionEntry> LoadPlaySessions(const std::wstring& gamePath,
    ...
    if (outGameName) {
        Utf8In mf(PlaySessionMetaPath(gamePath).c_str());
        if (mf.is_open()) {
            std::wstring p, n;
            std::getline(mf, p);
            std::getline(mf, n);
    ...
    Utf8In f(PlaySessionLogPath(gamePath).c_str());
