...
static void EtwDiag(const std::wstring& msg) {
    ...
    Utf8Out f(path.c_str(), std::ios::app);
    if (!f.is_open()) return;
    SYSTEMTIME st;
    GetLocalTime(&st);
    wchar_t ts[48];
...
bool EtwFps::Start(DWORD targetPid) {
    ...
    bool started = false;
    ULONG lastIndepResult = 0, lastNormalResult = 0;
    for (int attempt = 1; attempt <= 3 && !started; attempt++) {
...

        {
            ...
            swprintf_s(b, 160, L"StartTrace[NORMAL] attempt=%d result=%u",
                       attempt, result);
            EtwDiag(b);
            lastNormalResult = result;
            if (result == ERROR_SUCCESS) { started = true; break; }
        }

        // رفض الصلاحيات ليس عطلاً مؤقتاً: إعادة المحاولة لن تفيد وتؤخر تشغيل اللعبة ~ثانية
        if (!started && lastIndepResult == ERROR_ACCESS_DENIED && lastNormalResult == ERROR_ACCESS_DENIED) {
            EtwDiag(L"StartTrace: access denied (needs admin) — not retrying.");
            break;
        }
    }
